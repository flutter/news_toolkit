...
final _sourcePath = path.join('google_news_example${path.separator}');
...
final _workflowWorkingDirectoryRegExp = RegExp(
  r'\s+defaults:(.*?)google_news_example',
  multiLine: true,
  dotAll: true,
);
      ...
      if (path.isWithin(
        ...
        file.writeAsStringSync(
          contents
              ...
              .replaceAll('google_news_example/', ''),
        );
      }
      ...
      if (file.path == _targetCodeOwners) {
        file.writeAsStringSync(
          file.readAsStringSync().replaceFirst(
                ...
                '@bselwe @felangel @AnnaPS @Jan-Stepien',
                '{{code_owners}}',
              ),
        );
      }
